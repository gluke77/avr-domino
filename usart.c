...
static uint8_t		usart0_mode;
static uint8_t		usart0_int_mode;
static uint8_t		usart0_protocol;
...
void usart0_setprotocol_codenet(void)
{
	USART0_SET_8N1;
	usart0_protocol = USART_PROTOCOL_CODENET;
}
...
ISR(USART0_RX_vect)
{
	...
	case USART_PROTOCOL_SEC:
		...
		break;
	case USART_PROTOCOL_CODENET:
		usart0_rx_byte_codenet(byte);
		break;
	}
}
...
result_e usart0_cmd(uint8_t * req, uint8_t * ack, size_t ack_size, uint16_t delay)
{
	...
	else if (USART_PROTOCOL_CODENET == usart0_getprotocol())
	{
		if (CN_ETX == req[0])
...
void usart0_rx_byte_codenet(uint8_t byte)
{
	if (CN_ACK == byte)
	{
	...
	if (CN_ESC == usart0_inbuf[0] &&
		CN_EOT == usart0_inbuf[usart0_inbuf_pos-1])
	{
		usart0_msg_ready = 1;
		usart0_inbuf_pos = 0;
	}
	else if (CN_NAK == usart0_inbuf[0] &&
		4 == usart0_inbuf_pos)
	{
		usart0_msg_ready = 1;
		usart0_inbuf_pos = 0;
...
static uint8_t		usart1_mode;
static uint8_t		usart1_int_mode;
static uint8_t		usart1_protocol;
...
void usart1_setprotocol_codenet(void)
{
	USART1_SET_8N1;
	usart1_protocol = USART_PROTOCOL_CODENET;
}
...
result_e usart1_cmd(uint8_t * req, uint8_t * ack, size_t ack_size, uint16_t delay)
{
	...
	else if (USART_PROTOCOL_CODENET == usart1_getprotocol())
	{
		if (CN_ETX == req[0])
